...
import { EditOrder, CancelOrder, OrderRef, TradeFetch, OrderBookRef, RevertTrade } from './types';
...
export default class Orderbook {
    ...
    async editOrder(params: EditOrder): Promise<BigNumberish> {
        const sender = params.from || this.balancerManager;
        const orderbook = sender ? this.instance.connect(sender) : this.instance;
        return orderbook.editOrder(params.ref, params.amount);
    }
    ...
    async getTrade(params: TradeFetch): Promise<any> {
        const sender = params.from || this.balancerManager;
        const orderbook = sender ? this.instance.connect(sender) : this.instance;
        return orderbook.getTrade(params.from?.address, params.executionDate);
    }

    async removeTrade(params: RevertTrade): Promise<any> {
        const sender = params.from || this.balancerManager;
        const orderbook = sender ? this.instance.connect(sender) : this.instance;
        return orderbook.removeTrade(params.from?.address, params.executionDate);
    }
