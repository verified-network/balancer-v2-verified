...
describe('EnumerableMap', () => {
  ...
  describe('EnumerableIERC20ToUint256Map', () => {
    ...
    async function keyValuesMatch(store: { map: Contract }, keys: Array<string | BigNumber>, values :Array<string | BigNumber>, indexOfErrorCode: number): Promise<void> {
      ...
      values: Array<string | BigNumber>
        ...
        describe('setIndex', () => {
          ...
          it('reverts if index is invalid', async () => {
            await expect(store.map.setIndex(keyA, values.length, indexOfErrorCode)).to.be.revertedWith(indexOfErrorCode.toString());
          });
