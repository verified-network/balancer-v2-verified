...
describeForkTest('NoProtocolFeeLiquidityBootstrappingPoolFactory', 'mainnet', 14850000, function () {
  ...
  it('deploy a liquidity bootstrapping pool', async () => {
    ...
    pool = await task.instanceAt('NoProtocolFeeLiquidityBootstrappingPool', event.args.pool);
  ...
  it('authorized accounts can disable the factory', async () => {
    ...
    expect(await factory.isDisabled()).to.equal(true);
  });

  it('disabling prevents pool creation', async () => {
    await expect(
      factory.create('LBP', 'LBPT', tokens, initialWeights, swapFeePercentage, owner.address, swapEnabledOnStart)
    ).to.be.revertedWith('BAL#211'); // Errors.DISABLED
  });
});
=======
import hre from 'hardhat';
